    ...
    <div>
      ...
      */}
      <div className="mt-8 rounded-2xl border border-navy/10 bg-paper p-7 shadow-[0_30px_70px_-30px_rgba(0,0,0,0.55)] sm:p-9">
        ...
        {/*
          Below here used to be three stacked bordered boxes (the log, a
          divider-topped row of chips, a divider-topped input row, a
          divider-topped footnote), which is the visual grammar of a form:
          ruled-off fields, not a conversation. One recessed well for the
          transcript, chips floating free above the composer instead of
          fenced into their own strip, and a single pill-shaped composer that
          merges the field and the send action the way an actual chat
          product does, replace it. Nothing here is boxed off from anything
          else; spacing does the separating instead of rules.
        */}
        <div
          ref={logRef}
          role="log"
          ...
          className="mt-7 max-h-[22rem] overflow-y-auto rounded-xl bg-cream/70 px-5 py-5 shadow-[inset_0_1px_3px_rgba(23,42,58,0.08)]"
        >
          {turns.length === 0 ? (
        ...
        {/* Two informational chips and one conversion chip, floating free
            rather than fenced into their own bordered strip. */}
